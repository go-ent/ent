...
import (
	...
	"github.com/go-ent/ent/cmd/internal/base"
...
func migrate(c *gen.Config) {
	var (
		...
		oldCmd = []byte("github.com/go-ent/ent/cmd/entc")
	)
	buf, err := os.ReadFile(target)
	if err != nil || !bytes.Contains(buf, oldCmd) {
		return
	}
	_ = os.WriteFile(target, bytes.ReplaceAll(buf, oldCmd, []byte("github.com/go-ent/ent/cmd/ent")), 0644)
}
