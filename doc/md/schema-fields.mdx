...
[ValueScanner](https://pkg.go.dev/github.com/go-ent/ent/schema/field?tab=doc#ValueScanner) interface, or has an
...
[`entsql.Annotation`](https://pkg.go.dev/github.com/go-ent/ent@master/dialect/entsql#Annotation):
...
When a custom [`GoType`](#go-type) is being used, it must be convertible to the basic `string` type or it needs to implement the [ValueScanner](https://pkg.go.dev/github.com/go-ent/ent/schema/field#ValueScanner) interface.

The [EnumValues](https://pkg.go.dev/github.com/go-ent/ent/schema/field#EnumValues) interface is also required by the custom Go type to tell Ent what are the permitted values of the enum.
...
Implement the [EnumValues](https://pkg.go.dev/github.com/go-ent/ent/schema/field#EnumValues) interface.
```go
package property
...
```
The following example shows how to define an `Enum` field with a custom Go type that is not convertible to `string`, but it implements the [ValueScanner](https://pkg.go.dev/github.com/go-ent/ent/schema/field#ValueScanner) interface:
...
```
Implement also the [ValueScanner](https://pkg.go.dev/github.com/go-ent/ent/schema/field?tab=doc#ValueScanner) interface.
