...
```console
go run -mod=mod github.com/go-ent/ent/cmd/ent new User Group
```

:::note 
Please note, that some schema names (like `Client`) are not available due to 
[internal use](https://pkg.go.dev/github.com/go-ent/ent/entc/gen#ValidSchemaName). You can circumvent reserved names by using an 
